...
Per-slot and per-campaign rule sets that decide whether a candidate creative may serve on a candidate slot. Brand-safety profiles compose category filters and content rules (text-, image-, and metadata-derived). Decisioning consults brand-safety before returning a creative payload.
