...
Computes which campaigns drove which conversions and rolls the result up into brand- and retailer-facing reports. The current default attribution model is last-click within a 7-day window (ADR-0007). ROAS, CPA, and impression-share metrics are derived from attributed conversions.
...
The attribution pipeline SHALL roll up attributed conversion value over campaign, flight, and line-item and expose ROAS (return on ad spend) and CPA (cost per action) in the brand-facing reporting API.
