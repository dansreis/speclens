...
A retailer reviewer SHALL be able to transition a `submitted` creative to `approved` or `rejected`. A rejection MUST include a reason.
...
A brand user SHALL be able to address the rejection reason and resubmit the same creative. Resubmission MUST create a new revision; the rejected revision SHALL remain visible.
